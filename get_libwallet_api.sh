#!/bin/bash
MONERO_URL=https://github.com/Toklio-project/Toklio.git
...
git -C $MONERO_DIR checkout release-v0.14
...
# Build libwallet if no previous version file exists
    ...
    echo "Toklio/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove Toklio/lib/libwallet_merged.a to force rebuild"
    fi
fi

...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting tokliod " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM tokliod.exe");
#else
                QProcess::execute("pkill tokliod");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./tokliod status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    m_monerod = QApplication::applicationDirPath() + "/tokliod";
#endif

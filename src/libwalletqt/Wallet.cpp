...
class WalletListenerImpl : public  Toklio::WalletListener
{
public:
...
{
    QFuture<Toklio::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Toklio::Wallet::connected);
    QFutureWatcher<Toklio::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Toklio::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Toklio::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Toklio::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Toklio::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Toklio::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Toklio::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Toklio::optional<uint64_t>(), mixin_count,
                static_cast<Toklio::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Toklio::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Toklio::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Toklio::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bittoklio.log";
}
...
Wallet::Wallet(Toklio::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Toklio::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);

...
# Base environnement
        ...
        docker build -f android32.Dockerfile -t toklio-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t toklio-gui-android .
        docker create -it --name toklio-gui-android toklio-gui-android bash

# Get the apk

        docker cp toklio-gui-android:/opt/android/toklio-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/toklio-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp

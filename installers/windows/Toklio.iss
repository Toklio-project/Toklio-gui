...
; As of version 0.12.3.0, the Toklio .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Toklio.
...
Source: "bin\toklio-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Toklio wallet RPC interface implementation
...
; Filename: "{app}\toklio-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\Toklio');
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\toklio-project\toklio-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Toklio default location
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\toklio-wallet-cli.exe"; WorkingDir: "{userdocs}\Toklio\wallets"
...
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\toklio-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\toklio-project"; Flags: uninsdeletekeyifempty
